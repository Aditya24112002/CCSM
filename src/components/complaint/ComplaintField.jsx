...
import {
  ...
  nativeToDisplayDate,
  normalizeDate,
} from '../../features/complaint/dateUtils';
...
function ComplaintField({ definition, value, onChange, isHighlighted, isMissing, readOnly }) {
  ...
  return (
    <label
      className={`field ${wide ? 'field-wide' : ''} ${isHighlighted ? 'field-ai-highlight' : ''} ${isMissing ? 'field-missing' : ''} ${dateError ? 'field-error' : ''}`}
    >
      <span>{label}</span>
        ...
        {type === 'date' ? (
          <>
            ...
            <input
              ...
              type="date"
              value={displayToNativeDate(value)}
      ...
      {dateError && (
        <small className="field-error-text">Use a valid date in DD/MM/YYYY format.</small>
      )}
    </label>
  );
}
